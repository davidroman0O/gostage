...
func (b *Builder) Build() (Definition, error) {
	...
	if b.def.ID == "" {
		return Definition{}, errHelper.Newf(gostageerrors.ErrCodeInvalidArgument, "id is required")
	}
	seenStage := make(map[string]struct{})
	for _, stage := range b.def.Stages {
		if stage.ID != "" {
			if _, ok := seenStage[stage.ID]; ok {
				return Definition{}, errHelper.WithContext(errHelper.Newf(gostageerrors.ErrCodeInvalidArgument, "duplicate stage id %q", stage.ID), map[string]any{
					"stage_id": stage.ID,
				})
			}
	...
	for _, act := range s.stage.Actions {
		if act.ID != "" {
			if _, ok := actionIDs[act.ID]; ok {
				return Stage{}, errHelper.WithContext(errHelper.Newf(gostageerrors.ErrCodeInvalidArgument, "duplicate action id %s in stage %s", act.ID, s.stage.ID), map[string]any{
					"action_id": act.ID,
					"stage_id":  s.stage.ID,
				})
			}
		...
		if act.Ref == "" {
			return Stage{}, errHelper.WithContext(errHelper.Newf(gostageerrors.ErrCodeInvalidArgument, "action %s requires registry ref", act.ID), map[string]any{
				"action_id": act.ID,
				"stage_id":  s.stage.ID,
			})
		}
...
func (a *ActionBuilder) Build() (Action, error) {
	...
	if a.action.Ref == "" {
		return Action{}, errHelper.Newf(gostageerrors.ErrCodeInvalidArgument, "action ref required")
	}
	...
	for _, action := range stage.Actions {
		if action.ID != "" {
			if _, ok := actionIDs[action.ID]; ok {
				return errHelper.WithContext(errHelper.Newf(gostageerrors.ErrCodeInvalidArgument, "duplicate action id %s in stage %s", action.ID, stage.ID), map[string]any{
					"action_id": action.ID,
					"stage_id":  stage.ID,
				})
			}
		...
		if action.Ref == "" {
			return errHelper.WithContext(errHelper.Newf(gostageerrors.ErrCodeInvalidArgument, "action %s missing ref", action.ID), map[string]any{
				"action_id": action.ID,
				"stage_id":  stage.ID,
			})
		}
