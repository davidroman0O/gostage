...
// Manager wraps a state.Manager and emits telemetry events for state changes.
type Manager struct {
...
// If dispatcher is nil, the delegate is returned unchanged.
...
// WrapWithTelemetry wraps a state.Manager to emit telemetry events.
// This is an alias for Wrap to maintain backward compatibility with orchestrator package.
...
func (m *Manager) ActionProgress(ctx context.Context, workflowID, stageID, actionName string, progress int, message string) error {
	...
	m.emit(telemetry.Event{
		Kind:       telemetry.EventActionProgress,
		WorkflowID: workflowID,
		StageID:    stageID,
		ActionID:   actionName,
		Message:    message,
...
func (m *Manager) emit(evt telemetry.Event) {
	if m.dispatcher == nil {
		return
	}
	if evt.WorkflowID != "" {
	...
	if evt.SchemaVersion == 0 {
		evt.SchemaVersion = telemetry.DefaultEventSchema
	}
...
func (m *Manager) SuppressWorkflowEvents(workflowID string, kinds ...telemetry.EventKind) {
...
func (m *Manager) clearSuppressed(workflowID string) {
	if workflowID == "" {
		return
	}
	m.mu.Lock()
